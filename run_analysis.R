...
#   Check to see if all required files are present in working directory
if(!file.exists("UCI HAR Dataset/train/X_train.txt")) {
    stop("Data file not in current working directory")
}
if(!file.exists("UCI HAR Dataset/train/X_train.txt")) {
    stop("Data file not in current working directory")
}
...
train_measurements <- read.table("UCI HAR Dataset/train/X_train.txt", stringsAsFactors=FALSE)
...
test_measurements <- read.table("UCI HAR Dataset/test/X_test.txt", stringsAsFactors=FALSE)
...
#   Label dataset with appropriate variable names

var_names <- read.table("UCI HAR Dataset/features.txt", stringsAsFactors = FALSE) 

#   removing brackets and commas from variable names
